  ...
  return (
    <div className="space-y-1">
      ...
      <input
        ...
        disabled
        className="w-full rounded-lg border border-zinc-800 bg-zinc-900/50 px-3 py-2 text-sm text-zinc-300 placeholder-zinc-700 disabled:cursor-default disabled:opacity-70"
      />
    </div>
  )
}
...
export function BusinessProfileSidebar({ profile }: Props) {
  return (
    <aside className="flex h-full w-72 flex-col border-r border-zinc-800 bg-[#0a0a0a] p-4">
      {/* Logo Section */}
      <div className="mb-6 flex flex-col items-center">
        <div className="relative mb-3">
          {profile.logo ? (
            ...
            <div className="flex h-20 w-20 items-center justify-center rounded-full border-2 border-dashed border-zinc-700 bg-zinc-900">
              <Building2 size={28} className="text-zinc-600" />
            </div>
          )}
          <div className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border border-zinc-700 bg-zinc-800">
