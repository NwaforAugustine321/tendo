...
import {
  getHubPosition,
  isInVisibleArc,
} from "../../lib/workspace/radial-utils";
import {
  RADIAL_ANIMATION_MS,
...
import {
  DATA_SOURCES,
  type DataSource,
} from "../../lib/workspace/data-sources";
...
const radialActions = [
  ...
  },
  {
    ...
    type: "browse-folders" as const,
  },
  {
    label: "Connect Sources",
    icon: <Cable size={20} />,
    type: "connect-sources" as const,
  },
  {
    label: "Insights",
...
export function RadialMenu({ sidebarOpen = true }: { sidebarOpen?: boolean }) {
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      setRotationOffset((prev) =>
        clampRotation(prev - e.deltaY * SCROLL_SENSITIVITY),
      );
    },
    [clampRotation],
  );
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      touchStartY.current = e.touches[0].clientY;
      setRotationOffset((prev) =>
        clampRotation(prev + deltaY * TOUCH_SENSITIVITY),
      );
    },
    [clampRotation],
  );
  ...
  const handleActionClick = useCallback(
    (actionType: string) => {
      switch (actionType) {
        ...
        case "insights":
          closeRadialMenu();
          window.location.href = "/me/insights";
          break;
        case "settings":
          closeRadialMenu();
  ...
  const renderItems = () => {
    if (isActionsView) {
      return radialActions.map((action, index) => {
        const { angleDeg } = getHubPosition(
          index,
          radialActions.length,
          HUB_RADIUS,
          rotationOffset,
        );
    ...
    if (isFoldersView) {
      const viewFolders =
        radialMenuView.view === "folders" ? radialMenuView.folders : folders;
      const allItems = [
        ...
        if (item.kind === "folder" && item.folder) {
          ...
          return (
            ...
            >
              ...
              <span
                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#1a1a1a] border border-white/10 shadow-md group-hover:border-[#3ecf8e]/40 transition-colors ${colorClasses.text}`}
              >
        ...
        if (item.kind === "new-folder") {
          return (
            ...
            >
              ...
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#3ecf8e] text-white">
                <Plus size={18} />
              </span>
            </button>
          );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="absolute z-[105] overflow-hidden pointer-events-none"
          style={{
            left: -10,
            top: -(HUB_RADIUS + 70),
            width: HUB_RADIUS + 80,
        ...
        >
          <div
            ...
            style={{ left: 10, top: HUB_RADIUS + 70 }}
          >
            {renderItems()}
          </div>
        </div>
        ...
        {/* Scroll hints */}
        <div
          className="absolute z-10 pointer-events-none text-zinc-600 text-xs"
          style={{ left: HUB_RADIUS / 2, top: -(HUB_RADIUS + 20) }}
          aria-hidden="true"
        >
          ▲
        </div>
        <div
          className="absolute z-10 pointer-events-none text-zinc-600 text-xs"
          style={{ left: HUB_RADIUS / 2, top: HUB_RADIUS + 10 }}
          aria-hidden="true"
        >
          ▼
        </div>
