  ...
  return (
    ...
    >
      ...
      {!compact && (
        <>
          {/* Second ring */}
          <div className={clsx('absolute inset-[8%] rounded-full border transition-colors duration-300', isRecording ? 'border-red-500/20' : 'border-emerald-600/20')} />
          {/* Third dark ring */}
      ...
      <div className="relative z-10 flex flex-col items-center gap-0.5">
        <Mic
          size={compact ? 16 : 20}
          className={clsx('transition-colors duration-300', isRecording ? 'text-red-400' : 'text-emerald-400')}
        />
        {!compact && (
          <>
            <span className="text-sm font-medium text-white mt-1">
              {isRecording ? 'Listening...' : 'Ask Tendo AI'}
            </span>
            <span className={clsx('text-[10px] text-center leading-tight transition-colors duration-300', isRecording ? 'text-red-400/70' : 'text-zinc-500')}>
              {isRecording ? 'Tap to stop' : <>Ask anything about your<br />business</>}
            </span>
            <AudioLines size={14} className={clsx('mt-0.5 transition-colors duration-300', isRecording ? 'text-red-500/50' : 'text-emerald-600/50')} />
          </>
        )}
      </div>
    </button>
