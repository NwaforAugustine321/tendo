...
function sectionLabel(text: string) {
  return (
    <p className="px-0.5 text-[9px] font-medium uppercase tracking-wide text-zinc-400">
      {text}
    </p>
  );
}
...
function HomeBody({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-2 py-2">
      {sectionLabel("Home")}
      <div className="mt-1 space-y-0.5">
        <Link to="/me/home" onClick={onNavigate} className={navClass(false)}>
          Dashboard
        </Link>
        <Link to="/me/home" onClick={onNavigate} className={navClass(false)}>
          Profile
...
function RecentBody({ onNavigate }: { onNavigate?: () => void }) {
  return (
    <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-2 py-2">
      {sectionLabel("Recent")}
      <div className="mt-1 space-y-0.5">
        <Link to="/me/recent" onClick={onNavigate} className={navClass(false)}>
          Recent actions
        </Link>
        <Link to="/me/recent" onClick={onNavigate} className={navClass(false)}>
          Activity log
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center border-b border-zinc-800/60 px-2 py-1.5">
        <h2 className="min-w-0 flex-1 truncate text-xs font-medium tracking-wide text-zinc-400">
          {title}
        </h2>
      </div>
