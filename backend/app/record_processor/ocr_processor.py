...
from app.record_processor.ocr_errors import (
    OCREmptyResultError,
    OCRExtractionError,
    ...
    async def _call_api(self, image_data_url: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                print('Image processing response >>>>>>',[t.get('text_prediction',{}).get('text','')  for t in response.json().get('data',[])[0].get('text_detections',[])])
            except httpx.TimeoutException as e:
                raise OCRExtractionError(
                    f"NVIDIA OCR API request timed out: {e}"
                ) from e
            except httpx.RequestError as e:
                raise OCRExtractionError(
                    f"NVIDIA OCR API request failed: {e}"
                ) from e
        ...
        if response.status_code == 429:
            raise OCRExtractionError(
                f"NVIDIA OCR API rate limited (HTTP 429): {response.text}"
            )

        if response.status_code >= 500:
            raise OCRExtractionError(
                f"NVIDIA OCR API server error (HTTP {response.status_code}): {response.text}"
            )

        if response.status_code != 200:
            raise OCRExtractionError(
                f"NVIDIA OCR API error (HTTP {response.status_code}): {response.text}"
            )
