    ...
    **kwargs,
    ...
    if result.data:
        _event_writer.write(
            ...
            source="system",
            payload={"total": total, "type": transaction_type,
...
async def get_transactions(business_id: str, limit: int = 10, offset: int = 0, status: str = "", transaction_type: str = "", **kwargs) -> dict:
    ...
    q = client.table("transactions").select(
        "*, customers(name)").eq("business_id", business_id)
    if transaction_type:
...
async def get_transactions_summary(business_id: str, transaction_type: str = "", **kwargs) -> dict:
    ...
    client = get_client()
    q = client.table("transactions").select(
        "total, status, type").eq("business_id", business_id)
    if transaction_type:
        q = q.eq("type", transaction_type)
    result = q.execute()
    transactions = result.data or []
    total_revenue = sum(float(t["total"])
                        for t in transactions if t["status"] == "completed")
    return {
...
async def update_transaction(business_id: str, transaction_id: str, **kwargs) -> dict:
    ...
    client = get_client()
    valid_fields = ("total", "payment_type", "status",
                    "customer_id", "metadata", "type")
