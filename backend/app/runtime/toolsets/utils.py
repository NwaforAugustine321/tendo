    ...
    for param_name, param in signature.parameters.items():
        ...
        if get_origin(type_hint) is Annotated:
            ...
            annotated_field = next(
                (x for x in annotated_args[1:]
                 if isinstance(x, FieldInfo)), None
            )
    ...
    *,
    ...
    if isinstance(json_arguments, dict):
        ...
        try:
            ...
            raise ToolError(
                f"Error parsing arguments for `{fnc.info.name}`: {e}") from e
...
def to_langchain_tools(tools: list[Any] = []) -> list:
