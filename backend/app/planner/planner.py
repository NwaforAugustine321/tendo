...
def delegate_to_agents(session:dict | None = {}):
    ...
    async def _tool(
       ...
       shared_constraints: str = "",
        ...
        if has_dependencies:
           asyncio.create_task(_run_sequential(agent_dicts, shared_constraints,session=session))
        else:
           asyncio.create_task(_run_parallel(agent_dicts, shared_constraints,session=session))
